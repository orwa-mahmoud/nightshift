#!/usr/bin/env bash
# check-receipts.sh — validate a cited research report against its source manifest.
#
#   check-receipts.sh --project DIR --report PATH --manifest PATH [--output PATH...]
#
...
while [ $# -gt 0 ]; do
  case "$1" in
    --project)
      [ $# -ge 2 ] || { printf 'check-receipts: --project needs a value\n' >&2; exit 1; }
      PROJECT="$2"
      shift 2
      ;;
    --report)
      [ $# -ge 2 ] || { printf 'check-receipts: --report needs a value\n' >&2; exit 1; }
      REPORT="$2"
      shift 2
      ;;
    --manifest)
      [ $# -ge 2 ] || { printf 'check-receipts: --manifest needs a value\n' >&2; exit 1; }
      MANIFEST="$2"
      shift 2
      ;;
    --output)
      [ $# -ge 2 ] || { printf 'check-receipts: --output needs a value\n' >&2; exit 1; }
    ...
    -h | --help)
      ...
      exit 1
      ;;
    *) printf 'check-receipts: unknown argument: %s\n' "$1" >&2; exit 1 ;;
  esac
done

HOST="$(cd -P "$PROJECT" 2>/dev/null && pwd)" || {
  printf 'check-receipts: cannot cd to %s\n' "$PROJECT" >&2
  exit 1
}

fail() {
  printf 'check-receipts: %s\n' "$1" >&2
  exit 2
}
...
[ -n "$REPORT" ] || { printf 'check-receipts: --report is required\n' >&2; exit 1; }
[ -n "$MANIFEST" ] || { printf 'check-receipts: --manifest is required\n' >&2; exit 1; }
