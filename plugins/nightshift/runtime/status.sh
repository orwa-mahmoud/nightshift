...
declare PUNCH ARMED_FILE PARKING DRAFTING_TABLE WORK_ORDERS SNAGS MAP STOP_FILE SESSION_FILE WATCH_REASON LOG
...
fact "parked" "$(ns_status_entry_count "$PARKING")"
ns_status_entry_titles "$PARKING" 0 | while IFS= read -r line; do
  [ -n "$line" ] && fact "parked entry" "$line"
done

DRAFTS="$(ns_open_drafts "$DRAFTING_TABLE" 2>/dev/null)" || DRAFTS=0
ORDERS="$(ns_open_boxes_file "$WORK_ORDERS" 2>/dev/null)" || ORDERS=0
...
ns_status_entry_titles "$SNAGS" 3 | while IFS= read -r line; do
  [ -n "$line" ] && fact "snag" "$line"
done

fact "opportunities" "$(ns_status_opportunity_counts "$MAP")"
ns_status_building "$MAP" | while IFS="$(printf '\t')" read -r key value; do
...
fact "session" "$([ -f "$SESSION_FILE" ] && printf 'bound' || printf 'none')"
...
REASON_NOTE=""
case "$WATCHMAN_STATE" in alive*) ;; *) REASON_NOTE="; the watchman that recorded it is not running" ;; esac
if [ -f "$WATCH_REASON" ]; then
  REASON_CODE="$(ns_reason_code "$NS" 2>/dev/null)"
  fact "watch reason" "${REASON_CODE:-none}${REASON_CODE:+ ($(ns_reason_label "$REASON_CODE" 2>/dev/null)$REASON_NOTE)}"
else
...
ns_status_transitions "$LOG" 3 | while IFS= read -r line; do
