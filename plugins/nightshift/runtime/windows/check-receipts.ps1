...
function Fail-NSCheck {
    param([string]$Message)
    Write-NSCheckError "check-receipts: $Message"
    exit 2
}
...
try {
    ...
    Write-NSCheckError "check-receipts: cannot cd to $Project"
    exit 1
}

if ([string]::IsNullOrWhiteSpace($Report)) {
    Write-NSCheckError 'check-receipts: -Report is required'
    exit 1
}
if ([string]::IsNullOrWhiteSpace($Manifest)) {
    Write-NSCheckError 'check-receipts: -Manifest is required'
    exit 1
}
