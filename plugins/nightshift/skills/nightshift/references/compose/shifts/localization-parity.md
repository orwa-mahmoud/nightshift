...
- [ ] **Localization parity — repair objective key drift without inventing translations.**
  ...
  - A missing translation that requires language judgment is an owner decision in parking-lot.md:
    record the locale, source text, key, the decision needed, and the default this shift keeps: the
    key stays untranslated. Do not copy machine output into the catalog.
