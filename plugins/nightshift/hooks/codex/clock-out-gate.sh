...
if [ ! -f "$ARMED" ]; then codex_emit_release; exit 0; fi
...
if [ "$PUNCH_UNREADABLE" -ne 1 ]; then
  ns_gate_usage_sync "$NS" "$PROJECT_DIR" "$PUNCH" "$TICKED" "${CODEX_TRANSCRIPT_PATH:-}" || :
fi
...
if [ -n "$NS_CONTRACT_MOVED" ]; then
  ...
  codex_emit_block "$NS_CONTRACT_MOVED"
  exit 0
fi
