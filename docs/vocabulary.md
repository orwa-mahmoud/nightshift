...
| **work target** | `.nightshift/run/work-target` | the folder that receives inspection and verification — a Git repository in repository mode, a persistent non-Git folder in artifact mode |
| **work mode** | `.nightshift/run/work-mode` | `repository` or `artifact`; legacy repositories can omit it, but a non-Git folder needs Setup to record artifact mode |
...
| **process lease** | `.nightshift/run/.shift-lease` | transient ownership of the active shift process — each watchman recovery advances its generation, admitting the recovered worker and fencing stale processes on the same conversation without locking other tabs |
...
| **archive** | one folder per shift, `.nightshift/archive/<YYYY-MM-DD>/` by default and laid out like the live site; `archive.root` and `archive.layout` move and name it | filing of shipped items, the journal, handled snags, and copied artifact receipts. Filing is a copy. A ticked item's receipt leaves live storage once the shift has ended; an open item's receipt stays. Other records leave only when Archive names them. Missing or empty receipts create no dated receipts folder. |
...
| **work order** | `.nightshift/staging/work-orders.md` | a prepared job ticket — the item plus its hours, clock not running until the cut |
| **snag log** | `.nightshift/inbox/snag-log.md` | findings ledger across runs — cycle 4 never re-reports cycle 1 |
| **product research** | `.nightshift/product/product-research.md` | dated evidence about the product, users, comparable tools, and unmet needs; conclusions keep their source links |
| **opportunity map** | `.nightshift/product/opportunity-map.md` | ranked product opportunities with evidence, value, differentiation, effort, reversibility, risk, and an explicit status; its single `building` entry is the resumable current cycle |
| **parking lot** | `.nightshift/inbox/parking-lot.md` | decisions for the human — parked with a default chosen, the run continues |
