...
# Off Windows the run leaves out each block that needs ACLs, junctions, cmd.exe, Task Scheduler or
# powershell.exe, and names it, so every portable check still runs on every platform.
function Skip-WindowsOnly {
    param([Parameter(Mandatory = $true)][string]$What)
    $script:Skipped++
    Write-Host "  skipped off Windows: $What"
}
...
try {
    ...
    if (Test-NSWindows) {
        ...
        Skip-WindowsOnly 'the session identity ACL'
    }
    ...
    if (Test-NSWindows) {
        ...
        Skip-WindowsOnly 'the lease capability ACL'
    }
    ...
    if (Test-NSWindows) {
        ...
        Skip-WindowsOnly 'Task Scheduler registration'
    }
...
'@ | Set-Content -LiteralPath $agentStub -Encoding UTF8
        ...
        Skip-WindowsOnly 'native recovery, whose revival child runs powershell.exe'
    }
    ...
    if (Test-NSWindows) {
        ...
        Skip-WindowsOnly 'the pidfile-gone standdown, whose watchman starts through powershell.exe'
    }
...
'@ | Set-Content -LiteralPath $freshGuardStub -Encoding UTF8
        ...
        Skip-WindowsOnly 'the backoff and fresh-rung ladders, whose children run powershell.exe'
    }
...
'@ | Set-Content -LiteralPath $codexShim -Encoding ASCII
        ...
        Skip-WindowsOnly 'recovery through a codex.cmd shim'
    }
    ...
    if (Test-NSWindows) {
        ...
        Skip-WindowsOnly 'the live-process standby check, which reuses the native recovery fixture'
    }
    ...
    if (Test-NSWindows) {
        ...
        Skip-WindowsOnly 'the detached launcher, which starts powershell.exe'
    }
...
'@ | Set-Content -LiteralPath $clockOkStub -Encoding UTF8
    ...
    $skippedNote = if ($script:Skipped -gt 0) { ", $script:Skipped Windows-only blocks skipped" } else { '' }
    Write-Host "Windows-native verification passed ($script:Assertions assertions$skippedNote)."
